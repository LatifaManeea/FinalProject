#!/usr/bin/env bash
# One-time setup: makes sync_reel_to_supabase.py run automatically once a
# day via macOS's launchd, exactly like setup_daily_scrape.sh (VOX) and
# setup_daily_scrape_muvi.sh do - scraping Reel (Granada Mall) and
...
# not touch the existing VOX/Muvi schedules at all.
#
# Before running this for the first time:
#   1. Reel's one branch (Granada Mall, source='reel', source_code='0010')
#      must already exist in `branches` - see sync_reel_to_supabase.py's
#      --propose-branches output, already run once for this project.
#   2. scripts/.env must already exist with SUPABASE_URL and
#      SUPABASE_SERVICE_ROLE_KEY (the same file the VOX/Muvi jobs use).
#
# Then run this yourself from a normal Terminal (not through Claude):
#
#   cd path/to/final_project/scripts
#   chmod +x setup_daily_scrape_reel.sh
#   ./setup_daily_scrape_reel.sh
#
...
LABEL="com.ticked.reelsync"
PLIST_PATH="$HOME/Library/LaunchAgents/${LABEL}.plist"
LOG_PATH="$SCRIPT_DIR/reel_sync.log"
...
# Default: run once a day at 08:10 local time - five minutes after the
# Muvi job's default 08:05 (which is itself five minutes after VOX's
# 08:00), so none of the three fire at the same instant and their log
# files don't interleave. Change the Hour/Minute values below and
# re-run this script if you'd rather it ran at a different time.
HOUR=8
MINUTE=10
...
<dict>
    ...
    <array>
        ...
        <string>${SCRIPT_DIR}/sync_reel_to_supabase.py</string>
    </array>
...
echo "Each run scrapes Reel (Granada Mall) and upserts films/showtimes"
